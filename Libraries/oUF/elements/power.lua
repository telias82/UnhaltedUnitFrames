...
local function Update(self, event, unit)
	...
	* unit - the unit for which the update has been triggered (string)
	...
	-- Only call SetMinMaxValues when the range actually changes (saves work when
	-- power type/max is stable, e.g. warrior OOC with rage at 0)
	if(min ~= element._lastMin or max ~= element._lastMax) then
		element._lastMin = min
