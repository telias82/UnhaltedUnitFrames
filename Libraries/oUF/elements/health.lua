...
local function Update(self, event, unit)
	...
	* unit - the unit for which the update has been triggered (string)
	...
	do
		...
		element.values._damageAbsorb=UnitGetTotalAbsorbs(unit) or 0
		element.values._healAbsorb=(UnitGetTotalHealAbsorbs and UnitGetTotalHealAbsorbs(unit)) or 0
	end

	-- MoP Classic: GetMaximumHealth/GetCurrentHealth don't exist on stub
	local max = UnitHealthMax(unit)
	local cur = UnitHealth(unit)
	-- Only call SetMinMaxValues when max actually changes (UNIT_HEALTH fires every hit;
	-- max only changes on UNIT_MAXHEALTH, so this skips the call the vast majority of the time)
	if(max ~= element._lastMax) then
		element._lastMax = max
	...
	if(element.HealingAll or element.HealingPlayer or element.HealingOther or element.OverHealIndicator) then
		...
		if(element.OverHealIndicator) then
			element.OverHealIndicator:SetAlpha(healClamped and 1 or 0)
		end
	end
	...
	if(element.DamageAbsorb or element.OverDamageAbsorbIndicator) then
		...
		if(element.OverDamageAbsorbIndicator) then
			element.OverDamageAbsorbIndicator:SetAlpha(damageAbsorbClamped and 1 or 0)
		end
	end
	...
	if(element.HealAbsorb or element.OverHealAbsorbIndicator) then
		...
		if(element.OverHealAbsorbIndicator) then
			element.OverHealAbsorbIndicator:SetAlpha(healAbsorbClamped and 1 or 0)
		end
	end
