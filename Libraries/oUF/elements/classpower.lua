...
local POWER_ID_ARCANE_CHARGES = 16
local POWER_ID_CHI = 12
local POWER_ID_COMBO_POINTS = 4
local POWER_ID_ENERGY = 3
local POWER_ID_ESSENCE = 19
local POWER_ID_HOLY_POWER = 9
local POWER_ID_SOUL_SHARDS = 7
...
elseif(playerClass == 'WARLOCK') then
	...
	GetPowerUpdaters = function()
		if((C_SpecializationInfo and C_SpecializationInfo.GetSpecialization and C_SpecializationInfo.GetSpecialization() or (GetSpecialization and GetSpecialization())) == SPEC_WARLOCK_DESTRUCTION) then
...
local function Visibility(self, event, unit)
	...
	elseif(classAuraID) then
		if(not requireSpec or requireSpec == (C_SpecializationInfo and C_SpecializationInfo.GetSpecialization and C_SpecializationInfo.GetSpecialization() or (GetSpecialization and GetSpecialization()))) then
