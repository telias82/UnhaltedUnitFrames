...
local tagStrings = {
	...
	['arcanecharges'] = [[function()
		if((C_SpecializationInfo and C_SpecializationInfo.GetSpecialization and C_SpecializationInfo.GetSpecialization() or (GetSpecialization and GetSpecialization())) == SPEC_MAGE_ARCANE) then
			local num = UnitPower('player', 16)
			if(num > 0) then
				return num
			end
		end
	...
	['chi'] = [[function()
		if((C_SpecializationInfo and C_SpecializationInfo.GetSpecialization and C_SpecializationInfo.GetSpecialization() or (GetSpecialization and GetSpecialization())) == SPEC_MONK_WINDWALKER) then
			local num = UnitPower('player', 12)
			if(num > 0) then
				return num
			end
		end
	...
	['holypower'] = [[function()
		if((C_SpecializationInfo and C_SpecializationInfo.GetSpecialization and C_SpecializationInfo.GetSpecialization() or (GetSpecialization and GetSpecialization())) == SPEC_PALADIN_RETRIBUTION) then
			local num = UnitPower('player', 9)
			if(num > 0) then
				return num
			end
		end
	...
	['perhp'] = [[function(u)
		local h, hmax = UnitHealth(u), UnitHealthMax(u); return string.format('%d', hmax > 0 and (h/hmax*100) or 0)
	end]],

	['perpp'] = [[function(u)
		local p, pmax = UnitPower(u), UnitPowerMax(u); return string.format('%d', pmax > 0 and (p/pmax*100) or 0)
	end]],
...
-- Single master clock: one OnUpdate frame drives all tag update paths and
-- eventless unit frame updates. Eliminates the independent accumulator drift
-- between multiple per-timer OnUpdate frames that causes subtle frame pacing
-- desync during camera pans.
local UUF_masterTime = 0
local UUF_masterClock = CreateFrame('Frame')
UUF_masterClock:SetScript('OnUpdate', function(self, elapsed)
	UUF_masterTime = UUF_masterTime + elapsed

	-- 1. Event-driven tag flush (stringsToUpdate populated by eventFrame OnEvent)
	local lastFlush = self._lastFlush or 0
	if(UUF_masterTime - lastFlush >= eventTimerThreshold) then
		self._lastFlush = UUF_masterTime
	...
	-- 2. frequentUpdates timer tags (all intervals, fired from same tick)
	for timer, strings in next, timerFontStrings do
		local last = timerLastFired[timer] or 0
		if(UUF_masterTime - last >= timer) then
			timerLastFired[timer] = UUF_masterTime
			for fs in next, strings do
	...
	-- 3. Eventless unit frame updates (targettarget, focustarget, etc.)
	--    Registered by units.lua into Private.masterCallbacks before this file loads.
	local cbs = Private.masterCallbacks
	if(cbs) then
		for _, cb in next, cbs do
			if(UUF_masterTime - cb.lastFired >= cb.interval) then
				cb.lastFired = UUF_masterTime
				cb.func()
			end
		end
	end
...
local function enableTimer(timer)
	-- Ensure the strings table exists; master clock picks it up automatically
...
local function disableTimer(timer)
	-- Master clock naturally skips empty tables; no frame hide needed
end
...
local function getTagFunc(tagstr)
	...
	if(not func) then
		...
		func = function(self)
			...
			-- Dirty check: skip SetText entirely when the rendered string hasn't changed.
			-- During camera pans OOC with nothing happening, this eliminates all recomposite calls.
			local _result = string.format(format, unpack(buffer, 1, num))
			if(_result ~= self._uuf_lastText) then
				self._uuf_lastText = _result
