...
local function CastStart(self, event, unit)
	...
	-- MoP Classic: no UnitCastingDuration/UnitChannelDuration/castID/isEmpowered
	local direction = 0 -- elapsed
	local name, text, texture, startTime, endTime, isTradeSkill, _, notInterruptible, spellID = UnitCastingInfo(unit)
	local castID = spellID -- synthesize castID from spellID
	local duration
	if(name) then
	...
	-- MoP Classic: no SetTimerDuration; track times manually in OnUpdate
	element._duration  = duration
	element._startTime = startTime
	element._endTime   = endTime
	element._direction = direction
	element:SetMinMaxValues(0, duration or 1)
	if direction == 1 then
		element:SetValue(duration or 0)
	else
		element:SetValue(0)
	end

	if(element.Icon) then element.Icon:SetTexture(texture or FALLBACK_ICON) end
	if(element.Shield) then element.Shield:SetAlpha(notInterruptible and 1 or 0) end
...
local function CastUpdate(self, event, unit, _, _, castID)
	...
	-- MoP Classic: recalculate from UnitCastingInfo/UnitChannelInfo
	local direction = 0
	local name, startTime, endTime, duration
	...
	if(unit == 'player') then
		-- MoP: startTime already in seconds (divided above)
		local delta
		if(element.channeling) then
...
local function CastStop(self, event, unit, _, _, ...)
	...
	if(event == 'UNIT_SPELLCAST_STOP') then
		castID = ...
	elseif(false) then -- MoP: no empower events
...
local function CastInterruptible(self, event, unit)
	...
	if(element.Shield) then element.Shield:SetAlpha(element.notInterruptible and 1 or 0) end
...
local function onUpdate(self, elapsed)
	if(self.casting or self.channeling or self.empowering) then
		-- MoP Classic: calculate progress from stored _startTime/_endTime
		local now = GetTime()
		local duration = self._duration or 0
		local remaining
		if self._direction == 1 then
			-- channel: counts down
			remaining = math.max(0, (self._endTime or now) - now)
			self:SetValue(remaining)
		else
			-- cast: counts up
			local elapsed = now - (self._startTime or now)
			self:SetValue(math.min(elapsed, duration))
...
local function Enable(self, unit)
	...
	if(element and unit and not unit:match('%wtarget$')) then
		...
		if(not element.smoothing) then
			element.smoothing = (Enum and Enum.StatusBarInterpolation and Enum.StatusBarInterpolation.Immediate) or 0
		end
		...
		if(self.unit == 'player' and not (self.hasChildren or self.isChild or self.isNamePlate)) then
			-- MoP Classic: hide Blizzard casting bars
			if CastingBarFrame then CastingBarFrame:UnregisterAllEvents(); CastingBarFrame:Hide() end
			if PetCastingBarFrame then PetCastingBarFrame:UnregisterAllEvents(); PetCastingBarFrame:Hide() end
		end
