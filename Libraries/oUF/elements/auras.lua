...
local function UpdateTooltip(self)
	...
	-- MoP Classic: use stored index/type on button
	local owner = self:GetParent().__owner
	if self._auraIsHarmful then
		GameTooltip:SetUnitDebuff(owner.unit, self._auraIndex)
	else
		GameTooltip:SetUnitBuff(owner.unit, self._auraIndex)
	end
end
...
local function updateAura(element, unit, data, position)
	...
	button._auraIsHarmful  = data._auraIsHarmful
	...
	if(button.Overlay) then
		if(element.showType or (data.isHarmfulAura and element.showDebuffType) or (not data.isHarmfulAura and element.showBuffType)) then
			-- MoP Classic: C_UnitAuras.GetAuraDispelTypeColor doesn't exist.
			-- Look up debuff type directly from dispelColorCurve plain table.
			local debuffType = data.debuffType or data.dispelType or ''
			local dispelIndex = oUF.Enum.DispelType[debuffType] or oUF.Enum.DispelType.none
			local color = element.dispelColorCurve and element.dispelColorCurve[dispelIndex]
			if not color then color = element.dispelColorCurve and element.dispelColorCurve[oUF.Enum.DispelType.none] end
	...
	if(button.Stealable) then
		if(element.showStealableBuffs and not UnitCanCooperate('player', unit)) then
			button.Stealable:SetAlpha(data.isStealable and 1 or 0)
		else
...
local function processData(element, unit, data, filter)
	...
	-- MoP Classic: isPlayerAura is set in scanAuras from source unit
	-- data.isPlayerAura already set
	...
	if(self.Auras or self.Buffs or self.Debuffs) then
		...
		if(auras) then
			...
			if(not auras.dispelColorCurve) then
				-- MoP: plain table keyed by dispelIndex integer
				auras.dispelColorCurve = {}
				for _, dispelIndex in next, oUF.Enum.DispelType do
					if(self.colors.dispel[dispelIndex]) then
						auras.dispelColorCurve[dispelIndex] = self.colors.dispel[dispelIndex]
					end
				end
			end
		...
		if(buffs) then
			...
			if(not buffs.dispelColorCurve) then
				-- MoP: plain table keyed by dispelIndex integer
				buffs.dispelColorCurve = {}
				for _, dispelIndex in next, oUF.Enum.DispelType do
					if(self.colors.dispel[dispelIndex]) then
						buffs.dispelColorCurve[dispelIndex] = self.colors.dispel[dispelIndex]
					end
				end
			end
		...
		if(debuffs) then
			...
			if(not debuffs.dispelColorCurve) then
				-- MoP: plain table keyed by dispelIndex integer
				debuffs.dispelColorCurve = {}
				for _, dispelIndex in next, oUF.Enum.DispelType do
					if(self.colors.dispel[dispelIndex]) then
						debuffs.dispelColorCurve[dispelIndex] = self.colors.dispel[dispelIndex]
					end
				end
			end
