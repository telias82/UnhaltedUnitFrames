...
do
	...
	local function updateDriver(driver)
		if(IsLoggedIn()) then
			if C_NamePlate and C_NamePlate.SetNamePlateSize then C_NamePlate.SetNamePlateSize(driver.plateWidth or 200, driver.plateHeight or 30) end
			...
			if C_NamePlateManager and C_NamePlateManager.SetNamePlateHitTestInsets then
				C_NamePlateManager.SetNamePlateHitTestInsets((Enum and Enum.NamePlateType and Enum.NamePlateType.Enemy) or 0, enemyInset, enemyInset, enemyInset, enemyInset)
			end

			local friendlyInset = driver.friendlyNonInteractible and hitInset or -hitInset
			if C_NamePlateManager and C_NamePlateManager.SetNamePlateHitTestInsets then
				C_NamePlateManager.SetNamePlateHitTestInsets((Enum and Enum.NamePlateType and Enum.NamePlateType.Friendly) or 1, friendlyInset, friendlyInset, friendlyInset, friendlyInset)
			end
				...
				for name, value in next, driver.cvars do
					if(type(value) == 'table') then
						for bitmaskIndex, bitmaskValue in next, value do
							if C_CVar and C_CVar.SetCVarBitfield then C_CVar.SetCVarBitfield(name, bitmaskIndex, bitmaskValue) else SetCVar(name, bitmaskValue) end
						end
					else
						if C_CVar and C_CVar.SetCVar then C_CVar.SetCVar(name, value) else SetCVar(name, value) end
					end
				end
			end
		end
	...
	local function driverEventHandler(self, event, unit)
		...
		elseif(event == 'PLAYER_TARGET_CHANGED') then
			local nameplate = C_NamePlate and C_NamePlate.GetNamePlateForUnit('target')
