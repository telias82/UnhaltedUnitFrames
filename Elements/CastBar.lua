...
function UUF:CreateUnitCastBar(unitFrame, unit)
    ...
    SpellNameText:SetFont(UUF.Media.Font, SpellNameDB.FontSize, UUF.Media.FontFlag)
    ...
    DurationText:SetFont(UUF.Media.Font, DurationDB.FontSize, UUF.Media.FontFlag)
    if GeneralDB.Fonts.Shadow.Enabled then
    ...
    if CastBarDB.Enabled then
        ...
        local function UpdateNotInterruptibleOverlay(frameCastBar)
            if frameCastBar.NotInterruptibleOverlay and frameCastBar.notInterruptible ~= nil then
                frameCastBar.NotInterruptibleOverlay:SetAlpha(frameCastBar.notInterruptible and 1 or 0)
            end
        end

        unitFrame.Castbar.PostCastStart = function(frameCastBar)
            -- MoP Classic: GetSpellInfo returns values directly, not a table
            local spellName = GetSpellInfo(frameCastBar.spellID)
            if spellName then
    ...
    if CastBarDB.Enabled then
        ...
        if unitFrame.Castbar then
            ...
            if unitFrame.Castbar.Text then
                ...
                unitFrame.Castbar.Text:SetFont(UUF.Media.Font, SpellNameDB.FontSize, UUF.Media.FontFlag)
                if GeneralDB.Fonts.Shadow.Enabled then
            ...
            if unitFrame.Castbar.Time then
                ...
                unitFrame.Castbar.Time:SetFont(UUF.Media.Font, DurationDB.FontSize, UUF.Media.FontFlag)
                if GeneralDB.Fonts.Shadow.Enabled then
