...
-- Private.masterCallbacks is the shared registration table for the master clock
-- created in elements/tags.lua (which loads after this file). Each entry drives
-- one interval group of eventless unit frames, all fired from the same game tick.
if(not Private.masterCallbacks) then
	Private.masterCallbacks = {}
end

local function createOnUpdate(timer)
	-- Only register once per unique timer interval
	for _, cb in next, Private.masterCallbacks do
		if(cb.interval == timer) then return end
	end

	local objects = eventlessObjects[timer]
	table.insert(Private.masterCallbacks, {
		interval = timer,
		lastFired = 0,
		func = function()
