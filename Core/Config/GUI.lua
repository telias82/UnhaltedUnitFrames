...
local function CreateTextureSettings(containerParent)
    ...
    MouseoverStyleDropdown:SetList({["BORDER"] = "Border", ["OVERLAY"] = "Overlay", ["GRADIENT"] = "Gradient" })
    MouseoverStyleDropdown:SetLabel("Highlight Style")
    MouseoverStyleDropdown:SetValue(UUF.db.profile.Units.player.Indicators.Mouseover.Style)
    MouseoverStyleDropdown:SetRelativeWidth(0.5)
    MouseoverStyleDropdown:SetCallback("OnValueChanged", function(_, _, value) for _, unitDB in pairs(UUF.db.profile.Units) do if unitDB.Indicators.Mouseover and unitDB.Indicators.Mouseover.Enabled then unitDB.Indicators.Mouseover.Style = value end end UUF:UpdateAllUnitFrames() end)
    ...
    MouseoverHighlightSlider:SetValue(UUF.db.profile.Units.player.Indicators.Mouseover.HighlightOpacity)
    ...
    ForegroundOpacitySlider:SetValue(UUF.db.profile.Units.player.HealthBar.ForegroundOpacity)
    ...
    BackgroundOpacitySlider:SetValue(UUF.db.profile.Units.player.HealthBar.BackgroundOpacity)
...
local function CreateFrameSettings(containerParent, unit, unitHasParent, updateCallback)
    ...
    HeightSlider:SetSliderValues(1, 1000, 1)
...
local function CreateHealPredictionSettings(containerParent, unit, updateCallback)
    ...
    AbsorbHeightSlider:SetSliderValues(1, FrameDB.Height - 2, 1)
    ...
    AbsorbHeightSlider:SetDisabled(HealPredictionDB.Absorbs.MatchParentHeight)
    AbsorbSettings:AddChild(AbsorbHeightSlider)

    -- Position hardcoded to ATTACH (Attach To Missing Health)
    ...
    HealAbsorbHeightSlider:SetSliderValues(1, FrameDB.Height - 2, 1)
    ...
    HealAbsorbHeightSlider:SetDisabled(HealPredictionDB.HealAbsorbs.MatchParentHeight)
...
local function CreateCastBarSpellNameTextSettings(containerParent, unit, updateCallback)
    ...
    SpellNameXPosSlider:SetSliderValues(-1000, 1000, 1)
    ...
    SpellNameYPosSlider:SetSliderValues(-1000, 1000, 1)
...
local function CreateCastBarDurationTextSettings(containerParent, unit, updateCallback)
    ...
    DurationXPosSlider:SetSliderValues(-1000, 1000, 1)
    ...
    DurationYPosSlider:SetSliderValues(-1000, 1000, 1)
...
local function CreatePowerBarSettings(containerParent, unit, updateCallback)
    ...
    local DetachToggle = AG:Create("CheckBox")
    DetachToggle:SetLabel("Detach (Drag Freely)")
    DetachToggle:SetValue(PowerBarDB.Detached)
    DetachToggle:SetCallback("OnValueChanged", function(_, _, value) PowerBarDB.Detached = value updateCallback() end)
    DetachToggle:SetRelativeWidth(0.33)
    LayoutContainer:AddChild(DetachToggle)
    ...
    local DetachedWidthSlider = AG:Create("Slider")
    DetachedWidthSlider:SetLabel("Detached Width")
    DetachedWidthSlider:SetValue(PowerBarDB.DetachedWidth or 200)
    DetachedWidthSlider:SetSliderValues(10, 1000, 1)
    DetachedWidthSlider:SetRelativeWidth(0.33)
    DetachedWidthSlider:SetCallback("OnValueChanged", function(_, _, value) PowerBarDB.DetachedWidth = value updateCallback() end)
    LayoutContainer:AddChild(DetachedWidthSlider)
    ...
    local PowerBarTagsContainer = GUIWidgets.CreateInlineGroup(containerParent, "Power Bar Tags")

    local function SelectPowerBarTagTab(TabContainer, _, tagDB)
        TabContainer:ReleaseChildren()
        local tagData = PowerBarDB.PowerBarTags[tagDB]

        local TagSettingsContainer = GUIWidgets.CreateInlineGroup(TabContainer, "Tag Settings")

        local EditBox = AG:Create("EditBox")
        EditBox:SetLabel("Tag")
        EditBox:SetText(tagData.Tag)
        EditBox:SetRelativeWidth(0.5)
        EditBox:DisableButton(true)
        EditBox:SetCallback("OnEnterPressed", function(_, _, value)
            tagData.Tag = value
            EditBox:SetText(tagData.Tag)
            if unit == "boss" then UUF:UpdateBossFrames() else UUF:UpdateUnitPowerBarTag(UUF[unit:upper()], unit, tagDB) end
        end)
        TagSettingsContainer:AddChild(EditBox)

        local ColourPicker = AG:Create("ColorPicker")
        ColourPicker:SetLabel("Colour")
        ColourPicker:SetColor(tagData.Colour[1], tagData.Colour[2], tagData.Colour[3], 1)
        ColourPicker:SetFullWidth(true)
        ColourPicker:SetHasAlpha(false)
        ColourPicker:SetRelativeWidth(0.5)
        ColourPicker:SetCallback("OnValueChanged", function(_, _, r, g, b)
            tagData.Colour = {r, g, b}
            if unit == "boss" then UUF:UpdateBossFrames() else UUF:UpdateUnitPowerBarTag(UUF[unit:upper()], unit, tagDB) end
        end)
        TagSettingsContainer:AddChild(ColourPicker)

        local LayoutSettingsContainer = GUIWidgets.CreateInlineGroup(TabContainer, "Layout & Positioning")

        local AnchorFromDropdown = AG:Create("Dropdown")
        AnchorFromDropdown:SetList(AnchorPoints[1], AnchorPoints[2])
        AnchorFromDropdown:SetLabel("Anchor From")
        AnchorFromDropdown:SetValue(tagData.Layout[1])
        AnchorFromDropdown:SetRelativeWidth(0.5)
        AnchorFromDropdown:SetCallback("OnValueChanged", function(_, _, value)
            tagData.Layout[1] = value
            if unit == "boss" then UUF:UpdateBossFrames() else UUF:UpdateUnitPowerBarTag(UUF[unit:upper()], unit, tagDB) end
        end)
        LayoutSettingsContainer:AddChild(AnchorFromDropdown)

        local AnchorToDropdown = AG:Create("Dropdown")
        AnchorToDropdown:SetList(AnchorPoints[1], AnchorPoints[2])
        AnchorToDropdown:SetLabel("Anchor To")
        AnchorToDropdown:SetValue(tagData.Layout[2])
        AnchorToDropdown:SetRelativeWidth(0.5)
        AnchorToDropdown:SetCallback("OnValueChanged", function(_, _, value)
            tagData.Layout[2] = value
            if unit == "boss" then UUF:UpdateBossFrames() else UUF:UpdateUnitPowerBarTag(UUF[unit:upper()], unit, tagDB) end
        end)
        LayoutSettingsContainer:AddChild(AnchorToDropdown)

        local XPosSlider = AG:Create("Slider")
        XPosSlider:SetLabel("X Position")
        XPosSlider:SetValue(tagData.Layout[3])
        XPosSlider:SetSliderValues(-1000, 1000, 1)
        XPosSlider:SetRelativeWidth(0.33)
        XPosSlider:SetCallback("OnValueChanged", function(_, _, value)
            tagData.Layout[3] = value
            if unit == "boss" then UUF:UpdateBossFrames() else UUF:UpdateUnitPowerBarTag(UUF[unit:upper()], unit, tagDB) end
        end)
        LayoutSettingsContainer:AddChild(XPosSlider)

        local YPosSlider = AG:Create("Slider")
        YPosSlider:SetLabel("Y Position")
        YPosSlider:SetValue(tagData.Layout[4])
        YPosSlider:SetSliderValues(-1000, 1000, 1)
        YPosSlider:SetRelativeWidth(0.33)
        YPosSlider:SetCallback("OnValueChanged", function(_, _, value)
            tagData.Layout[4] = value
            if unit == "boss" then UUF:UpdateBossFrames() else UUF:UpdateUnitPowerBarTag(UUF[unit:upper()], unit, tagDB) end
        end)
        LayoutSettingsContainer:AddChild(YPosSlider)

        local FontSizeSlider = AG:Create("Slider")
        FontSizeSlider:SetLabel("Font Size")
        FontSizeSlider:SetValue(tagData.FontSize)
        ...
        FontSizeSlider:SetCallback("OnValueChanged", function(_, _, value)
            tagData.FontSize = value
            if unit == "boss" then UUF:UpdateBossFrames() else UUF:UpdateUnitPowerBarTag(UUF[unit:upper()], unit, tagDB) end
        end)
        LayoutSettingsContainer:AddChild(FontSizeSlider)

        local TagSelectionContainer = GUIWidgets.CreateInlineGroup(TabContainer, "Tag Selection")
        ...
        HealthTagDropdown:SetCallback("OnValueChanged", function(_, _, value)
            local cur = tagData.Tag
            tagData.Tag = (cur and cur ~= "") and (cur .. "[" .. value .. "]") or ("[" .. value .. "]")
            EditBox:SetText(tagData.Tag)
            if unit == "boss" then UUF:UpdateBossFrames() else UUF:UpdateUnitPowerBarTag(UUF[unit:upper()], unit, tagDB) end
            HealthTagDropdown:SetValue(nil)
        end)
        ...
        PowerTagDropdown:SetCallback("OnValueChanged", function(_, _, value)
            local cur = tagData.Tag
            tagData.Tag = (cur and cur ~= "") and (cur .. "[" .. value .. "]") or ("[" .. value .. "]")
            EditBox:SetText(tagData.Tag)
            if unit == "boss" then UUF:UpdateBossFrames() else UUF:UpdateUnitPowerBarTag(UUF[unit:upper()], unit, tagDB) end
        ...
        NameTagDropdown:SetCallback("OnValueChanged", function(_, _, value)
            local cur = tagData.Tag
            tagData.Tag = (cur and cur ~= "") and (cur .. "[" .. value .. "]") or ("[" .. value .. "]")
            EditBox:SetText(tagData.Tag)
            if unit == "boss" then UUF:UpdateBossFrames() else UUF:UpdateUnitPowerBarTag(UUF[unit:upper()], unit, tagDB) end
        ...
        MiscTagDropdown:SetCallback("OnValueChanged", function(_, _, value)
            local cur = tagData.Tag
            tagData.Tag = (cur and cur ~= "") and (cur .. "[" .. value .. "]") or ("[" .. value .. "]")
            EditBox:SetText(tagData.Tag)
            if unit == "boss" then UUF:UpdateBossFrames() else UUF:UpdateUnitPowerBarTag(UUF[unit:upper()], unit, tagDB) end
    ...
    local PowerBarTagTabGroup = AG:Create("TabGroup")
    PowerBarTagTabGroup:SetLayout("Flow")
    PowerBarTagTabGroup:SetFullWidth(true)
    PowerBarTagTabGroup:SetTabs({
        { text = "Tag One", value = "PowerBarTagOne" },
        { text = "Tag Two", value = "PowerBarTagTwo" },
    })
    PowerBarTagTabGroup:SetCallback("OnGroupSelected", SelectPowerBarTagTab)
    PowerBarTagTabGroup:SelectTab("PowerBarTagOne")
    PowerBarTagsContainer:AddChild(PowerBarTagTabGroup)
    ...
    function RefreshPowerBarGUI()
        if PowerBarDB.Enabled then
            ...
            GUIWidgets.DeepDisable(PowerBarTagsContainer, false)
            ...
            GUIWidgets.DeepDisable(PowerBarTagsContainer, true)
        end
    end
...
local function CreateSecondaryPowerBarSettings(containerParent, unit, updateCallback)
    ...
    local DetachToggle = AG:Create("CheckBox")
    DetachToggle:SetLabel("Detach (Drag Freely)")
    DetachToggle:SetValue(SecondaryPowerBarDB.Detached)
    DetachToggle:SetCallback("OnValueChanged", function(_, _, value) SecondaryPowerBarDB.Detached = value updateCallback() end)
    DetachToggle:SetRelativeWidth(0.33)
    LayoutContainer:AddChild(DetachToggle)
    ...
    local DetachedWidthSlider = AG:Create("Slider")
    DetachedWidthSlider:SetLabel("Detached Width")
    DetachedWidthSlider:SetValue(SecondaryPowerBarDB.DetachedWidth or 200)
    DetachedWidthSlider:SetSliderValues(10, 1000, 1)
    DetachedWidthSlider:SetRelativeWidth(0.33)
    DetachedWidthSlider:SetCallback("OnValueChanged", function(_, _, value) SecondaryPowerBarDB.DetachedWidth = value updateCallback() end)
    LayoutContainer:AddChild(DetachedWidthSlider)
    ...
    local SecondaryPowerBarTagsContainer = GUIWidgets.CreateInlineGroup(containerParent, "Secondary Power Bar Tags")

    local function SelectSecondaryPowerBarTagTab(TabContainer, _, tagDB)
        TabContainer:ReleaseChildren()
        local tagData = SecondaryPowerBarDB.PowerBarTags[tagDB]
        ...
        EditBox:SetCallback("OnEnterPressed", function(_, _, value)
            tagData.Tag = value
            EditBox:SetText(tagData.Tag)
            UUF:UpdateUnitSecondaryPowerBarTag(UUF[unit:upper()], unit, tagDB)
        end)
        TagSettingsContainer:AddChild(EditBox)
    ...
    local SecondaryPowerBarTagTabGroup = AG:Create("TabGroup")
    SecondaryPowerBarTagTabGroup:SetLayout("Flow")
    SecondaryPowerBarTagTabGroup:SetFullWidth(true)
    SecondaryPowerBarTagTabGroup:SetTabs({
        { text = "Tag One", value = "PowerBarTagOne" },
        { text = "Tag Two", value = "PowerBarTagTwo" },
    })
    SecondaryPowerBarTagTabGroup:SetCallback("OnGroupSelected", SelectSecondaryPowerBarTagTab)
    SecondaryPowerBarTagTabGroup:SelectTab("PowerBarTagOne")
    SecondaryPowerBarTagsContainer:AddChild(SecondaryPowerBarTagTabGroup)

    function RefreshSecondaryPowerBarGUI()
        if SecondaryPowerBarDB.Enabled then
            GUIWidgets.DeepDisable(LayoutContainer, false, Toggle)
            GUIWidgets.DeepDisable(ColourContainer, false, Toggle)
            GUIWidgets.DeepDisable(SecondaryPowerBarTagsContainer, false)
            ...
            GUIWidgets.DeepDisable(LayoutContainer, true, Toggle)
            GUIWidgets.DeepDisable(ColourContainer, true, Toggle)
            GUIWidgets.DeepDisable(SecondaryPowerBarTagsContainer, true)
        end
    end
...
local function CreateSpecificAuraSettings(containerParent, unit, auraDB)
    ...
    CountXPosSlider:SetSliderValues(-1000, 1000, 1)
    ...
    CountYPosSlider:SetSliderValues(-1000, 1000, 1)
...
local function CreateAuraSettings(containerParent, unit)
    ...
    local function SelectAuraTab(AuraContainer, _, AuraTab)
        ...
        local _t = CreateFrame("Frame") _t:SetScript("OnUpdate", function(self) self:SetScript("OnUpdate", nil) RefreshFontSizeSlider() end)
        containerParent:DoLayout()
    end
...
local function CreateGlobalSettings(containerParent)
    ...
    TagIntervalSlider:SetSliderValues(0.5, 10, 0.5)
...
local function CreateUnitSettings(containerParent, unit)
    ...
    local function SelectUnitTab(SubContainer, _, UnitTab)
        ...
        UUF:SetDetachedHoldersDraggable(false)
        ...
        elseif UnitTab == "PowerBar" then
            local function enablePowerHolder()
                if unit == "boss" then
                    for i = 1, UUF.MAX_BOSS_FRAMES do
                        local f = UUF["BOSS"..i]
                        if f and f.PowerHolder then f.PowerHolder:EnableMouse(true) end
                    end
                else
                    local f = UUF[unit:upper()]
                    if f and f.PowerHolder then
                        f.PowerHolder:EnableMouse(true)
                        UUF:SetNudgeTarget(f.PowerHolder, UUF.db.profile.Units[unit].PowerBar, unit:sub(1,1):upper()..unit:sub(2).." Power Bar", function()
                            UUF:LayoutPowerBar(f, unit)
                        end)
                    end
                end
            end
            enablePowerHolder()
            CreatePowerBarSettings(SubContainer, unit, function()
                if unit == "boss" then UUF:UpdateBossFrames() else UUF:UpdateUnitPowerBar(UUF[unit:upper()], unit) end
                enablePowerHolder()
            end)
        elseif UnitTab == "SecondaryPowerBar" then
            local function enableSecondaryHolder()
                local f = UUF[unit:upper()]
                if f and f.SecondaryPowerHolder then
                    f.SecondaryPowerHolder:EnableMouse(true)
                    UUF:SetNudgeTarget(f.SecondaryPowerHolder, UUF.db.profile.Units[unit].SecondaryPowerBar, unit:sub(1,1):upper()..unit:sub(2).." Secondary Power Bar", function()
                        UUF:LayoutSecondaryPowerBar(f, unit)
                    end)
                end
            end
            enableSecondaryHolder()
            CreateSecondaryPowerBarSettings(SubContainer, unit, function()
                UUF:UpdateUnitSecondaryPowerBar(UUF[unit:upper()], unit)
                enableSecondaryHolder()
            end)
...
local function CreateProfileSettings(containerParent)
    ...
    for i = 1, numSpecs do
        ...
        if C_SpecializationInfo and C_SpecializationInfo.GetSpecializationInfo then
            _, specName = C_SpecializationInfo.GetSpecializationInfo(i)
        elseif GetSpecializationInfo then
            _, specName = GetSpecializationInfo(i)
        end
